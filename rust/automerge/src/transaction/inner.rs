...
impl TransactionInner {
    ...
    /// Batch put multiple key-value pairs into a map object in a single splice.
    /// Returns OpIds for each entry (in the order of the input `entries`).
    ///
    /// For fresh objects (no existing keys), all ops are inserted in a single
    /// splice call across all 21 columns, which is dramatically faster than
    /// individual put/put_object calls.
    ///
    /// For objects that already have keys, falls back to individual operations.
    pub(crate) fn local_map_ops_batch(
        &mut self,
        doc: &mut Automerge,
        patch_log: &mut PatchLog,
        obj: &ObjMeta,
        entries: Vec<(String, OpType)>,
    ) -> Result<Vec<OpId>, AutomergeError> {
        if entries.is_empty() {
            return Ok(vec![]);
        }

        // Sort entries by key to match map key ordering, preserving original indices
        let mut sorted_entries: Vec<(usize, String, OpType)> = entries
            .into_iter()
            .enumerate()
            .map(|(i, (k, v))| (i, k, v))
            .collect();
        ...
        // Check the object's current range
        let range = doc.ops().scope_to_obj(&obj.id);
        if range.start != range.end {
            // Object already has keys - fall back to one-by-one
            let n = sorted_entries.len();
            let mut result_ids = vec![OpId::new(0, 0); n];
            // Restore original order for processing
            sorted_entries.sort_by_key(|e| e.0);
            for (orig_idx, key, action) in sorted_entries {
                if let Some(id) = self.local_map_op(doc, patch_log, obj, key, action)? {
        ...
        // Fresh object - batch all ops in one splice
        let start_pos = range.end;
        let start_pending = self.pending.len();
        let n = sorted_entries.len();
        let mut result_ids = vec![OpId::new(0, 0); n];

        for (i, (orig_idx, key, action)) in sorted_entries.into_iter().enumerate() {
            let id = self.next_id();
            result_ids[orig_idx] = id;

            let resolved_action = crate::op_set2::op_set::ResolvedAction::VisibleUpdate(action);
            let op = TxOp::map(id, *obj, start_pos + i, resolved_action, key, vec![]);
